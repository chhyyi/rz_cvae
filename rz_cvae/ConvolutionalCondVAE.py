"""
It is modified from copy of src/ConvolutionalCondVAE.py from github repo. 'EleMisi/ContionalVAE'

- changes: l1 regularization included for all dense, conv. layers.

- Cautions: unit and reshape of decoder's first two layers
There are Dense, Reshape layer at Decoder. 

Unit of Dense layer should be (final output dimension * latent_dim)
It is 4*4*256, as 64, 64 image go through 4 Conv2D layer in original version. Same in this modification with 1024, 1024 input image and additional 4 MaxPool2D layers.

Reshape layer : (4, 4, latent_dim) in default inputs. 
